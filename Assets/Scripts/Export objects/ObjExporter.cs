...
namespace MyNameSpace
{
    public class ObjExporter : MonoBehaviour
    {
        ...
        private static async Task MaterialsToFile(Dictionary<string, ObjMaterial> materialList, string folder, string filename)
        {
            await Task.Run(() =>
            {
        ...
        private static async Task MeshToFile(MeshFilter mf, string folder, string filename, string textureFolderPath)
        {
            ...
            using (StreamWriter sw = new StreamWriter(folder + Path.DirectorySeparatorChar + filename + ".obj"))
            {
                ...
                await Task.Run(() => sw.Write(MeshToString(mf, materialList, textureFolderPath)));
            }

            await MaterialsToFile(materialList, folder, filename);
        }
        ...
        public static async Task ExportSelectedObj(string materialName, MeshFilter[] meshFilters, string exportFileName, string textureFolderPath)
        {
            ...
            {
                await ExportMeshesToFile(filteredMeshFilters.ToArray(), targetFolder, exportFileName, textureFolderPath);
            }
        }

        private static async Task ExportMeshesToFile(MeshFilter[] meshFilters, string folder, string filename, string textureFolderPath)
        {
            Dictionary<string, ObjMaterial> materialList = PrepareFileWrite();
            ...
            using (StreamWriter sw = new StreamWriter(folder + Path.DirectorySeparatorChar + filename + ".obj"))
            {
                ...
                for (int i = 0; i < meshFilters.Length; i++)
                {
                    ...
                    var task = Task.Run(async () =>
                    {
                        ...
                        try
                        {
                            sw.Write(MeshToString(meshFilters[localIndex], materialList, textureFolderPath));
                        }
                        finally
                        {
                            semaphore.Release();
